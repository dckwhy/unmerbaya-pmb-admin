                                    ...
                                    <div class="card-block">
                                        <div class="table-responsive">
                                            <table id="zero-configuration" class="display table nowrap table-striped table-hover" style="width:100%">
                                                ...
                                                <tbody>
                                                    <?php
                                                    $get = $this->db->get('data_home')->result();
                                                    foreach ($get as $value) { ?>
                                                    <tr>
                                                        <td><?= $value->id ?></td>
                                                        <td><span><img class="mr-2" src="<?= base_url('prabotan/image/home/'.@$value->banner) ?>" alt=""></span></td>
                                                        <td>
                                                            <?= get_edit_delete_js($value->id) ?>
                                                        </td>
                                                    </tr> 
                                                     <?php }
                                                    ?>
...
function get_detail (id) {  
    window.location = ('<?= base_url() ?>admin/home/detail/'+id)
}
function get_edit (id) {
    window.location = ('<?= base_url() ?>admin/home/edit/'+id) 
}
</script>
<script>
...
function act_delete(id){
    where_value = id;
    where_field = 'id';
    table_name  = 'data_home';
    $('.loading').show();
    $.ajax({
        type :"post",  
        url : "<?php echo base_url('admin/home/delete_data') ?>",  
        cache :false,
