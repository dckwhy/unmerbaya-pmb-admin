                                    ...
                                    <div class="card-block">
                                        <div class="table-responsive">
                                            <table id="zero-configuration" class="display table nowrap table-striped table-hover" style="width:100%">
                                                ...
                                                <tbody>
                                                    <?php
                                                    $get_jp = $this->db->get('data_jadwal_pendaftaran')->result();
                                                    foreach ($get_jp as $value) { ?>
                                                    <tr>
                                                        <td><?= $value->id ?></td>
                                                        <td><?= limit_text($value->content, 10) ?></td>
                                                        <td>
...
function get_detail (id) {  
    window.location = ('<?= base_url() ?>admin/jadwal_pendaftaran/detail/'+id)
}
function get_edit (id) {
    window.location = ('<?= base_url() ?>admin/jadwal_pendaftaran/edit/'+id) 
}
</script>
<script>
...
function act_delete(id){
    where_value = id;
    where_field = 'id';
    table_name  = 'data_jadwal_pendaftaran';
    $('.loading').show();
    $.ajax({
        type :"post",  
        url : "<?php echo base_url('admin/Jadwal_pendaftaran/delete_data') ?>",  
        cache :false,
